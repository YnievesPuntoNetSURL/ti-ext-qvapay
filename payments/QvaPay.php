...
namespace YnievesDotNetTeam\QvaPay\Payments;
...
class QvaPay extends BasePaymentGateway
{
    use EventEmitter;
    use PaymentHelpers;
    ...
    {
        return [
            'qvapay_return_url' => 'processReturnUrl',
            'qvapay_cancel_url' => 'processCancelUrl',
        ];
    }
    ...
    {
        $gateway = Omnipay::create('QvaPay');

        $gateway->setAppId($host->app_id);
        $gateway->setAppSecret($host->app_secret);
        $gateway->setTestMode($host->app_mode);
    ...
    {
        $cancelUrl = $this->makeEntryPointUrl('qvapay_cancel_url').'/'.$order->hash;
        $returnUrl = $this->makeEntryPointUrl('qvapay_return_url').'/'.$order->hash;
